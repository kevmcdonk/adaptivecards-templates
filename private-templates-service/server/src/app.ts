...
if (process.env.ACMS_APP_INSIGHTS_INSTRUMENTATION_KEY)
  appInsights
    .setup(
      "InstrumentationKey=" + process.env.ACMS_APP_INSIGHTS_INSTRUMENTATION_KEY
    )
    .start();
if (process.env.USER_APP_INSIGHTS_INSTRUMENTATION_KEY)
  appInsights
    .setup(
      "InstrumentationKey=" + process.env.USER_APP_INSIGHTS_INSTRUMENTATION_KEY
    )
    .start();
...
let mongoDB = new ACMS.MongoDBProvider(
  { connectionString: process.env.ACMS_DB_CONNECTION },
  "en"
);
mongoDB.connect().then((res: any) => {
  if (res.success) {
    const mongoClient = {
